...
import { getSheetTemplate, updateSheetTemplate } from '@/lib/data'
...
export default function EditTemplatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    getSheetTemplate(id).then(t => {
      if (t) {
        setName(t.name)
        setDescription(t.description || '')
        setFields(t.fields.map(f => ({ ...f, optionsRaw: f.options?.join(', ') || '' })))
      }
      setLoading(false)
    })
  }, [id])
  ...
  const updateField = (fid: string, key: string, value: string) => {
    setFields(prev => prev.map(f => f.id === fid ? { ...f, [key]: value } : f))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await updateSheetTemplate(id, { name, description, fields: cleanFields })
    router.push('/sheets')
    setSaving(false)
  }
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto dark:bg-gray-950 min-h-screen">
      <div className="flex items-center gap-3 mb-6 mt-2 md:mt-0">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Edit Template</h1>
      </div>
          ...
          <div className="space-y-3">
            {fields.map((field, idx) => (
              <div key={field.id} className="flex gap-2 items-start bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3">
                <div className="pt-2.5 text-gray-300 dark:text-gray-600 shrink-0"><GripVertical size={16} /></div>
                <div className="flex-1 space-y-2">
                  ...
                  {field.type === 'select' && (
                    <input type="text" value={field.optionsRaw} onChange={e => updateField(field.id, 'optionsRaw', e.target.value)}
